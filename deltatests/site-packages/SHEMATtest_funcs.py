...
def compilequick(
    ...
    compilation_script = (std.deltatests_dir + '/' + 'compilequick.sh'
                          if not git_branch == 'cmake' else
                          std.deltatests_dir + '/' + 'compilequick_cmake.sh')
    # Flags for gmake/cmake
    flags = (flags_dict['gmake']
             if not git_branch == 'cmake' else flags_dict['cmake'])
    ...
    fnew = open(model_dir + '/' + 'compilequick_run.sh', 'w')
    fnew.write(fcontentwrite)
    fnew.close()

    # Change file permission
    os.chmod(model_dir + '/' + 'compilequick_run.sh', 0770)

    # Run compilequick_run.sh
    comp_out_file = open(model_dir + '/' + 'compilequick.out', 'w')
    rm.run_script(
        model_dir,
        './' + 'compilequick_run.sh',
