...
class Tests_fw_bas_simpleConvection(unittest.TestCase):
    def test_comp_equal(self):

        fhdf1 = h5py.File(std.shemat_suite_models_dir +
                          '/fw_bas_simpleConvection' + std.result_dir +
                          '/convection_simple_final.h5')
        comp_array_desired = np.array(fhdf1['comp'])
        fhdf1.close()

        fhdf2 = h5py.File(
            std.shemat_suite_models_dir +
            '/fw_bas_simpleConvection/convection_simple_final.h5')
...
def local_suite():
    local_suite = unittest.makeSuite(
        Tests_fw_bas_simpleConvection, prefix='test')
    return local_suite
