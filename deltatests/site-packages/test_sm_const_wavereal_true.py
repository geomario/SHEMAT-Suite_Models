...
class Tests_sm_const_wavereal_true(unittest.TestCase):
    def test_conc0001_equal(self):

        fhdf1 = h5py.File(std.shemat_suite_models_dir +
                          '/sm_const_wavereal_true' + std.result_dir +
                          '/samples_output/WAVEREAL_TRUE_E0_1.h5')
        conc0001_array_desired = np.array(fhdf1['conc0001'])
        fhdf1.close()

        fhdf2 = h5py.File(
            std.shemat_suite_models_dir +
            '/sm_const_wavereal_true/samples_output/WAVEREAL_TRUE_E0_1.h5')
...
def local_suite():
    local_suite = unittest.makeSuite(
        Tests_sm_const_wavereal_true, prefix='test')
    return local_suite
